/*! angular-lookup-directive 21-11-2015 */

!function () {
    "use strict";
    var a = ["$http", "$q"], b = function (a, b) {
        var c = '<div><input type="text" ng-model="value" class="ruaghain-lookup" ng-keyup="onKeyUp($event)"><ul ng-keydown="onKeyDown($event)"><li ng-repeat="record in foundRecords"><a href="#" ng-click="onItemSelect(record)" ng-keyup="onItemKeyUp($event, record)">{{record[lookupTextField]}}</a></li></ul></div><div ng-show="addRecord"><button id="btnSave" ng-click="saveLookup()">Save</button><button id="btnCancel" ng-click="cancel()">Cancel</button></div>', d = function (c, d, e, f) {
            var g = d.find("input");
            f.$formatters.push(function (a) {
                ...
                var b = a.which;
                27 === b ? c.clearResults() : !c.searching && (b > 64 && 91 > b || b > 96 && 123 > b || 40 === b) && c.search()
                ...
                c.searching = !0, c.findRestData(g).then(function (a) {
                    c.addRecord = c.lookupAllowInsert && ("undefined" == typeof a || 0 === a.length), c.foundRecords = a
                })
