...
!function () {
    ...
    var a = ["$http", "$q"], b = function (a, b) {
        var c = '<div><input type="text" ng-model="value" class="ruaghain-lookup" ng-keyup="onKeyUp($event)"><ul ng-keydown="onKeyDown($event)"><li ng-repeat="record in foundRecords"><a href="#" ng-click="onItemSelect(record)">{{record[lookupTextField]}}</a></li></ul></div><div ng-show="addRecord"><button id="btnSave" ng-click="saveLookup()">Save</button><button id="btnCancel" ng-click="cancel()">Cancel</button></div>', d = function (c, d, e, f) {
            var g = d.find("input");
